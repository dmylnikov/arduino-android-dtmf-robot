...
// Androide >= 4.2 requires authentication for an adb connection
// This response sends a dummy public key. On the phone you
// have to approve the key each time the cable is connected
...
static void adb_printMessage(adb_message * message)
{
	...
	{
	case A_OKAY:
		Serial.print("OKAY message ");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_CLSE:
		Serial.print("CLSE message ");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_WRTE:
		Serial.print("WRTE message ");// [%lx] %ld %ld, %ld bytes\n", message->command, message->arg0, message->arg1, message->data_length);
		break;
	case A_CNXN:
		Serial.print("CNXN message ");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_SYNC:
		Serial.print("SYNC message ");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	case A_OPEN:
		Serial.print("OPEN message ");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	default:
		Serial.print("WTF message ");// [%lx] %ld %ld\n", message->command, message->arg0, message->arg1);
		break;
	}
	Serial.print("[");
	Serial.print(message->command, HEX);
        ...
        Serial.print(", ");
        Serial.println(message->data_length);
}
#endif
...
#ifdef DEBUG
	Serial.print("SEND >> "); adb_printMessage(&message);
#endif
...
boolean ADB::pollMessage(adb_message * message, boolean poll)
{
	...
	{
#ifdef DEBUG
                Serial.print("Broken message, magic mismatch, ");
                Serial.print(bytesRead);
                Serial.println(" bytes");
                Serial.println(message->command, HEX);
// 		Serial.printf("Broken message, magic mismatch, %d bytes\n", bytesRead);
		return false;
#endif
	}
